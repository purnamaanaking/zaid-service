...
class GoogleCalendarInboundSyncTest extends TestCase
{
    ...
    public function test_remote_cancelled_google_event_does_not_recreate_soft_deleted_linked_task(): void
    {
        Http::fake([
            'https://www.googleapis.com/calendar/v3/calendars/primary/events*' => Http::response([
                'items' => [[
        ...
        $this->assertSame(1, Task::withTrashed()->where('user_id', $user->id)->count());

        $link->refresh();
        $this->assertSame('cancelled', $link->remote_status);
